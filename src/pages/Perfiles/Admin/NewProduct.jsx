...
const NewProduct = ({ user }) => {
        ...
        const fetchProductos = async () => {
            ...
            try {
                ...
                if (debouncedSearchNombre && debouncedSearchNombre.trim() !== '') {
                    ...
                    const isSkuCandidate = !/\s/.test(q) && q.length >= 3 && q.length <= 50; // heurística: no espacios, tamaño razonable

                    if (isSkuCandidate) {
                        try {
                            const skuResp = await axiosInstance.get(`/api/products/sku/${encodeURIComponent(q)}`, { headers });
                ...
                if (result.products && Array.isArray(result.products)) {
                    ...
                    const productosFinal = (debouncedSearchNombre && debouncedSearchNombre.trim() !== '')
                        ? productosObtenidos.filter(p => p.item && p.item.toLowerCase().includes(debouncedSearchNombre.toLowerCase()))
                        : productosObtenidos;
                    ...
                    setTotalPaginas(pagination.totalPages || Math.ceil((pagination.total || productosFinal.length) / productsPerPage));
                    setTotalProductos(pagination.total || productosFinal.length);
                } else if (Array.isArray(result)) {
                    // Fallback por si el backend devuelve un array directamente
                    const productosFinal = (debouncedSearchNombre && debouncedSearchNombre.trim() !== '')
                        ? result.filter(p => p.item && p.item.toLowerCase().includes(debouncedSearchNombre.toLowerCase()))
                        : result;
    ...
    const handleSubmitProduct = async (e) => {
        ...
        try {
            ...
            formData.append("item", productForm.item);
            formData.append("descripcion", productForm.descripcion);
            formData.append("marca", productForm.marca);
            ...
            formData.append("iva", productForm.iva);
    ...
    const handleUpdateProduct = async (e) => {
        ...
        if (!productForm.sku || !productForm.item || !productForm.descripcion ||
            !productForm.categoria || !productForm.subcategoria || productForm.stock === 0 || productForm.precio === "" || productForm.precio <= 0) {
            Swal.fire({
                icon: "error",
                title: "Error de validación",
        ...
        try {
            ...
            if (productForm.imagenes.length > 0) {
                const formData = new FormData();
                formData.append('sku', productForm.sku);
                formData.append('item', productForm.item);
                formData.append('descripcion', productForm.descripcion);
                ...
                formData.append('iva', parseFloat(productForm.iva) || 0);
                formData.append('estado', productForm.estado);

                // Agregar archivos
                productForm.imagenes.forEach((file) => {
                    if (file) {
                        formData.append('imagen', file);
                    }
                });
                ...
                console.log('Actualizando producto (FormData), productForm:', productForm);

                for (const entry of formData.entries()) console.log('formData', entry[0], entry[1]);
                ...
                const productData = {
                    ...
                    iva: parseFloat(productForm.iva) || 0,
                    estado: productForm.estado
                };

                await axiosInstance.put(`/api/products/${editingProduct}`, productData, {
                    headers: headers,
                });
            }
            ...
            if (result.products && Array.isArray(result.products)) {
                setProductos(result.products);
                const pagination = result.pagination || {};
                setTotalPaginas(pagination.totalPages || Math.ceil((pagination.total || 0) / productsPerPage));
            ...
            const errorMessage = error.response?.data?.message || error.message || "No se pudo actualizar el producto";
            Swal.fire({
                icon: "error",
                title: "Error",
                text: errorMessage,
    ...
    const handleDeleteProduct = async (productoId) => {
        ...
        if (confirmResult.isConfirmed) {
            try {
                ...
                setProductos(prev =>
                    prev.filter(p => p._id !== productoId)
                );
            } catch (error) {
                ...
                Swal.fire({
                    icon: "error",
                    title: "Error",
                    text: error.response?.data?.message || error.message || "No se pudo eliminar el producto.",
                    confirmButtonColor: "#d33"
                });
            }
        }
                    ...
                    <div className="form-group">
                        ...
                        {productForm.imagen.length > 0 && (
                            <div style={{ marginTop: '10px' }}>
                                <p style={{ fontSize: '0.9rem', color: '#438796', fontWeight: 'bold' }}>
                                    {productForm.imagen.length} imagen(es) seleccionada(s):
                                </p>
                                <ul style={{ fontSize: '0.85rem', color: '#666', marginTop: '5px', paddingLeft: '20px' }}>
            ...
            <div className="admin-section">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', gap: '12px', flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                        ...
                        <button
                            type="button"
                            onClick={() => { setSearchNombre(''); setDebouncedSearchNombre(''); setCurrentPage(1); }}
                    ...
                    {debouncedSearchNombre && (
                        <div style={{ fontSize: '0.9rem', color: '#666' }}>
                            Resultados para: <strong style={{ color: '#438796' }}>{debouncedSearchNombre}</strong>
                        </div>
                    )}
                </div>
                            ...
                            {!Array.isArray(productos) || productos.length === 0 ? (
                                ...
                                productos.map((producto) => (
                                    <tr key={producto._id || producto.id}>
                                        ...
                                        <td>
                                            <span
                                                className={`role-badge ${producto.estado === "activo" ? "role-admin" : "role-user"
                                                    }`}
                                            >
                        ...
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                            ...
                            <div style={{ display: 'flex', gap: '4px' }}>
                                {Array.from({ length: totalPaginas }, (_, i) => i + 1).map((numero) => {
                                    ...
                                    if (
                                        ...
                                        return (
                                            <button
                                                ...
                                                style={{
                                                    padding: '8px 12px',
                                                    backgroundColor: currentPage === numero ? '#438796' : 'white',
                                    ...
                                    } else if (numero === currentPage - 2 || numero === currentPage + 2) {
                                        return (
                                            <span key={numero} style={{ padding: '8px', color: '#666' }}>
                                                ...
                                            </span>
                                        );
                                    }
                            ...
                            <button
                                ...
                                style={{
                                    padding: '8px 16px',
                                    backgroundColor: currentPage === totalPaginas ? '#ccc' : '#438796',
                                    color: 'white',
                                    border: 'none',
                                    borderRadius: '4px',
